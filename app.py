...
@st.cache_data
def load_color_map():
    """Loads bricklink_colors.csv and maps BrickLink ID to its Name."""
    file_path = "bricklink_colors.csv"
    if os.path.exists(file_path):
        try:
            df = pd.read_csv(file_path)
            # Standardized to your new BrickLink columns
...
try:
    ...
    for item in items:
        ...
        if rem_node is not None and rem_node.text:
            ...
            # Map Color ID to Name (Ensuring integer conversion)
            ...
            # Map Part ID to Name from Parts.txt
                    ...
                    for cond_type, col in zip(['N', 'U'], [c1, c2]):
                        with col:
                            ...
                            for p in items_in_cond:
                                # Updated display using p['color'] mapped from BrickLink ID

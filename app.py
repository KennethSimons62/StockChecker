...
# --- 4. SIDEBAR: PROFILE COMMANDER & EDITOR ---
with st.sidebar:
...
if not st.session_state.get('xml_data'):
    st.warning("⚪ No Store Data Loaded. Go to the HOME HUB to upload your XML.")
    st.stop()
    ...
    with tabs[idx]:
        ...
        for n in range(int(cat['start']), int(cat['end']) + 1):
            ...
            if umatches:
                ...
                label = "EMPTY SLOT" if total_parts == 0 else f"{total_parts} Parts"
                
                with st.expander(f"{icon} {pref}{n} — [{label}]"):
                    for h_n, data in umatches.items():
                        st.markdown(f"**📍 Hole {h_n}** | `{data['state']}`")

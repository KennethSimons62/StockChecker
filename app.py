...
if 'active_profile' not in st.session_state:
    st.session_state.active_profile = "Default_Store"
...
selected_p = st.sidebar.selectbox("Active Store Profile:", profile_list, index=profile_list.index(st.session_state.active_profile) if st.session_state.active_profile in profile_list else 0)
...
with st.sidebar.expander("➕ Create New Profile"):
    new_name = st.text_input("New Profile Name", placeholder="e.g. Warehouse_B")
    if st.button("Create Profile"):
        if new_name:
            save_profile_file(new_name, load_profile_file("Default_Store"))
...
# File Upload Handling (In-Memory)
if st.session_state.xml_data is None:
    uploaded_file = st.file_uploader("Upload your BrickLink store.xml file to begin:", type="xml")
    if uploaded_file:
        st.session_state.xml_data = uploaded_file.getvalue()
        st.rerun()
    st.stop()

if st.button("🔄 Upload Different Store"):
    st.session_state.xml_data = None
    st.rerun()
...
for i, cat in enumerate(st.session_state.temp_categories):
    with st.sidebar.expander(f"📁 Edit {cat['name']}", expanded=False):
        ...
        st.session_state.temp_categories[i]['is_wall'] = st.checkbox("4-digit (Wall)", value=cat.get('is_wall', False), key=f"w_{i}")
        if st.button("🗑️ Remove Section", key=f"del_{i}"):
...
if st.sidebar.button("💾 SAVE PROFILE", type="primary", use_container_width=True):
    ...
    st.sidebar.success(f"Saved to {st.session_state.active_profile}!")
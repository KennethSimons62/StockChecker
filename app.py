                    ...
                    for cond_type, col in zip(['N', 'U'], [c1, c2]):
                        with col:
                            ...
                            for p in items_in_cond:
                                ...
                                st.divideraimport streamlit as st
import xml.etree.ElementTree as ET
import re
import json
import os
...
@st.cache_data
def load_color_map():
    """Loads colors.csv and maps Color ID to its Name."""
    if os.path.exists("colors.csv"):
        try:
            # Map 'id' (int) to 'name' (str)
...
def load_internal_catalog():
    ...
    if os.path.exists("Parts.txt"):
        try:
            # BrickLink catalog format is tab-separated
            df_ref = pd.read_csv("Parts.txt", sep='\t', encoding='latin1')
            # Map 'Number' (Part ID) to its full descriptive 'Name'
            return dict(zip(df_ref.iloc[:, 2].astype(str), df_ref.iloc[:, 3]))
        except: return {}
    return {}
...
try:
    ...
    for item in items:
        ...
        if rem_node is not None and rem_node.text:
            ...
            # Map Color ID to Name
            color_node = item.find("COLOR")
            color_id = int(color_node.text) if color_node is not None else -1
            p_color = COLOR_LOOKUP.get(color_id, f"Color {color_id}")
            
            # Map Part ID to Name/Description
            p_desc = CATALOG_LOOKUP.get(p_id, "Description Not Found")
                ...
                with st.expander(f"🔴 Conflict: Container {drawer}", expanded=is_exp):
                    ...
                    for cond_type, col in zip(['N', 'U'], [c1, c2]):
                        with col:
                            ...
                            st.markdown(f"### {'🆕 NEW' if cond_type == 'N' else '📜 USED'} ({len(items_in_cond)} items)")
                            for p in items_in_cond:
                                # Highlight Color and Description for easy error trapping
                                st.markdown(f"**{p['desc']}**")
                                st.markdown(f"**Color:** {p['color']}")
                                st.caption(f"📍 Hole: {p['loc']} | 🆔 Part: {p['id']} | 🔢 Qty: {p['qty']}")
                                st.divider()
